...
class Arena:
    ...
    def create_step_record(self):
        ...
        record.hidden_target = torch.zeros(opt["game_nagents"], opt["model_rnn_layers"], opt["bs"],
                                           opt["model_rnn_size"])
    ...
    def run_episode(self, agents, train_mode=False):
        ...
        while step < episode_steps and episode.ended.sum() < opt["bs"]:
            ...
            for i in range(1, opt["game_nagents"] + 1):
                ...
                # Precious Communication Messages
                comm = None
                if opt["comm_enabled"]:
                    comm = episode.step_records[step].comm.clone()
                    comm_temp = torch.zeros(opt["bs"], 1, opt["game_comm_bits"])
                    for b in range(opt["bs"]):
                        if step > 0 and self.game.get_active_agent()[b][step].item() == agent_idx:
                            comm_temp[b] = comm[b][self.game.get_active_agent()[b][step-1]-1]
                        else:
                            comm_temp[b] = torch.zeros(1, opt["game_comm_bits"])
                    comm = comm_temp.clone()
