				...
				for step in range(b_steps):
					...
					if record.a_t[b][i].item() > 0:
						if record.terminal[b].item() > 0:
							...
							td_action += r_t
							if (td_count_lambda + 1) % td_lambda == 0:
								td_action += opt.gamma * q_next_max - q_a_t
					...
					if not opt.model_dial and record.a_comm_t[b][i].item() > 0:
						...
						if record.terminal[b].item() > 0:
							...
							td_comm += r_t
							if (td_count_lambda + 1) % td_lambda == 0:
								td_comm += opt.gamma * q_next_max - q_comm_t
